...
std::unique_ptr<const Packet> Packet::Deserialize(const std::span<uint8_t> buffer) {
    auto expected_crc32 = calculateCrc32(std::span<uint8_t>(buffer.data(), buffer.size() - sizeof(uint32_t)));
...
std::vector<uint8_t> Packet::Serialize(std::unique_ptr<const Packet> &packet) {
	...
	auto crc = htonl(calculateCrc32(std::span<uint8_t>(returnVector.data(), returnVector.size() - sizeof(uint32_t))));
...
std::unique_ptr<const Packet> Packet::CreatePacket(
	...
	auto packetSize = kMinPacketSize + data.size();
	...
	packet->ack = ack;
	packet->type = type;
	packet->data_length = static_cast<uint16_t>(data.size());
